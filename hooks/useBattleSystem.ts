...
export const useBattleSystem = () => {
    ...
    const lastHpMultiplierRef = useRef<number>(
        INITIAL_SETTINGS.globalHpMultiplier,
    );

    // Performance: Analytics history for debugging (Downloadable)
    const perfHistoryRef = useRef<
        { t: number; e: number; u: number; v: number; f: number }[]
    >([]);

    // Dynamic HP Scaling is now handled in PASS 0 of updateSimulation for maximum responsiveness
    const updateSettingsRef = useCallback(
        (key: keyof SimulationSettings, value: any) => {
    ...
    // Pre-allocated position tuple to avoid [...position] spreading in hot loop
    const _dmgPos: [number, number, number] = [0, 0, 0];
    ...
    const accumulateDamage = (
        ...
        // NOTE: Removed O(N) Array.find() attacker search — damage stats are
        // already tracked directly by the caller in updateSimulation.
    ...
    const markBaseHpDirty = () => {
        baseHpDirtyRef.current = true;
    };
    ...
    const flushKillEvents = (now: number) => {
        if (
            killEventBufferRef.current.length > 0 &&
            now - lastKillFlushRef.current > 150
        ) {
    ...
    const triggerAirstrike = useCallback((side: "player" | "enemy") => {
        const targets = unitsRef.current.filter(
            (u) => u && u.type !== side && !u.isDying,
        );
        targets.forEach((u) => {
            if (u?.id) {
    ...
    const spawnUnit = useCallback(
        (
            ...
            const userActiveUnits = unitsRef.current.filter(
                (u) => u && u.userName === userName && !u.isDying,
            ).length;
            if (
                !isBoss &&
                !isTestingBot &&
                useStore.getState().gameMode !== "TRAINING" &&
                userActiveUnits >= 3
            )
                return;

            // No setState needed, we rely completely on unitsRef in engine
            const teamUnits = unitsRef.current.filter(
                (u) => u && u.type === type && !u.isDying,
            );
            if (!isBoss && teamUnits.length >= towerConfigRef.current.maxUnits)
                return;

            statsRef.current.unitsSpawned[userName] =
                (statsRef.current.unitsSpawned[userName] || 0) + 1;

            let stats = getUnitStats(
                level,
            ...
            const unitAttackCooldown =
                baseCooldown / (c.attack_speed_mult || 1.0);
    ...
    const updateSimulation = useCallback(
        (delta: number) => {
            ...
            const physicsDelta =
                Math.min(0.05, delta) * (settings.timeScale || 1.0);
                    ...
                    const isBase =
                        ...
                        if (isBase) {
                            ...
                            if (target && target.hp > 0 && !target.isDying) {
                                const tData = unitDataRef.current.get(
                                    h.targetId,
                                );
                                if (tData) {
                                    ...
                                    const attackerClass =
                                        h.attackerClass || "fighter";
                                    const victimClass =
                                        target.unitClass || "fighter";
                                    const stats = statsRef.current;

                                    if (stats.classStats[attackerClass])
                                        stats.classStats[
                                            attackerClass
                                        ].damageDealt += h.damage;
                                    if (stats.classStats[victimClass])
                                        stats.classStats[
                                            victimClass
                                        ].damageTaken += h.damage;

                                    if (h.attackerName) {
                                        if (h.attackerId.startsWith("p")) {
                                            // Check attacker team
                                            stats.teamSummary.player.totalDamage +=
                                                h.damage;
                                        } else {
                                            stats.teamSummary.enemy.totalDamage +=
                                                h.damage;
                                        }
                                    }

                                    // Lifesteal/SpellVamp for delayed hits (Ranged)
                                    const attacker = unitIndexRef.current.get(
                                        h.attackerId,
                                    );
                                    if (attacker) {
                                        const isMage =
                                            h.attackerClass === "mage";
                                        const healMult = isMage
                                            ? attacker.spellVamp
                                            : attacker.lifesteal;
            ...
            if (now - lastReactSyncRef.current > 1500) {
                for (let i = unitsRef.current.length - 1; i >= 0; i--) {
                    ...
                    if (
                        ...
                        u.isDying &&
                        u.deathTime &&
                        now - u.deathTime > CORPSE_DESPAWN_MS
                    ) {
            ...
            // ---- PASS 1: Weather Multipliers & Environmental Context (PRE-CALCULATED) ----
            const weather = weatherRef.current;
            const wConfig = WEATHER_CONFIG[weather] as any;
            const wMults = wConfig.multipliers || {};
            const globalSpeedMult = wMults.globalSpeedMultiplier || 1.0;
            const globalDmgMult = wMults.globalDamageMultiplier || 1.0;
            const globalCooldownMult = wMults.globalAttackCooldown || 1.0;
            ...
            for (let i = 0; i < unitsRef.current.length; i++) {
                ...
                // Optimization: Pre-calculated weather multipliers for this unit class
                const classMults = wMults[u.unitClass] || {};
                const weatherSpeedMult = (classMults.move_speed_mult || 1.0) * globalSpeedMult;
                const weatherAtkSpeedMult = classMults.attack_speed_mult || 1.0;
                const weatherDmgMult = (classMults.atk || 1.0) * globalDmgMult;
                const weatherCooldownMult = globalCooldownMult;
                ...
                // Commit Combat State
                const currentTarget_lookup = u.targetId
                ...
                if (currentTarget) {
                    ...
                    // --- COMMIT BASE DAMAGE ---
                    const weatherAdjCooldown =
                        settingsRef.current.globalAttackCooldown *
                        weatherCooldownMult;
                    const baseAttackCooldown =
                        (u.isBoss
                            ? weatherAdjCooldown * 0.7
                            : weatherAdjCooldown) / weatherAtkSpeedMult;
                    if (simNow - uData.lastAttackTime > baseAttackCooldown) {
                        const dummyTargetStats = {
                            physicalDefense: 0,
                            magicDefense: 0,
                        } as any;
                        const { damage: rawDamage } = calcDamage(
                            u,
                            dummyTargetStats,
                            u.unitClass === "mage",
                        );
                ...
                if (
                    currentTarget &&
                    (currentTarget.hp <= 0 || currentTarget.isDying)
                ) {
                ...
                if (currentTarget) {
                    ...
                    if (tData) {
                        const dSq =
                            (uData.position[0] - tData.position[0]) ** 2 +
                            (uData.position[2] - tData.position[2]) ** 2;
                        const dynamicRangeSq =
                            (u.range + (u.id.charCodeAt(0) % 5) * 0.1) ** 2;

                        // --- ASSASSIN TELEPORT LOGIC ---
                        if (
                            u.unitClass === "assassin" &&
                            simNow - uData.lastBlinkTime >
                                ASSASSIN_BLINK_COOLDOWN
                        ) {
                            if (
                                dSq <
                                ...
                                if (v) {
                                    const dx =
                                        tData.position[0] - uData.position[0];
                                    const dz =
                                        tData.position[2] - uData.position[2];
                                    ...
                                    uData.untargetableUntil =
                                        simNow + ASSASSIN_INVUL_MS;
                                    uData.status = "attacking";
                                }
                            }
                        }
                        ...
                        if (
                            ...
                            const weatherAdjCooldown =
                                u.attackCooldown * weatherCooldownMult;
                            const finalAtkCooldown =
                                weatherAdjCooldown / weatherAtkSpeedMult;

                            if (
                                simNow - uData.lastAttackTime >
                                finalAtkCooldown
                            ) {
                                const teamColor =
                                    u.type === "player"
                                ...
                                if (
                                    ...
                                    const { damage: rawDmg } = calcDamage(
                                        u,
                                        currentTarget,
                                        u.unitClass === "mage",
                                    );
                                    ...
                                    if (availableSlot) {
                                        ...
                                        availableSlot.color =
                                            u.unitClass === "marksman"
                                                ? "#ffcc00"
                                                : teamColor;
                                        availableSlot.targetId =
                                            currentTarget.id;
                                        (availableSlot as any).isBullet =
                                            u.unitClass === "marksman";
                                    }
                                } else {
                                    // MELEE HIT
                                    ...
                                    const healMult =
                                        u.lifesteal + u.spellVamp * 0.5;
                                    if (healMult > 0) {
                                        u.hp = Math.min(
                                            u.maxHp,
                                            u.hp + finalDmg * healMult,
                                        );
                                        uData.hp = u.hp;
                                    }
                                    ...
                                    if (currentTarget.hp <= 0) {
                                        u.targetId = undefined;
                                        uData.victoryPauseUntil =
                                            now +
                                            settingsRef.current.victoryPauseMs;
                                        uData.status = "idle";
                                        addKillEvent(
                                            u.userName,
                                        ...
                                        // Analytics: Death tracking
                                        const stats = statsRef.current;
                                        if (stats.classStats[u.unitClass])
                                            stats.classStats[
                                                u.unitClass
                                            ].kills += 1;
                                        if (u.type === "player") {
                                            stats.playerKills[u.userName] =
                                                (stats.playerKills[
                                                    u.userName
                                                ] || 0) + 1;
                                            stats.teamSummary.player.totalKills += 1;
                                ...
                                statsRef.current.damageDealt[u.userName] =
                                    (statsRef.current.damageDealt[u.userName] ||
                                        0) + finalDmg;
                                if (u.type === "player") {
                                    statsRef.current.playerDamage[u.userName] =
                                        (statsRef.current.playerDamage[
                                            u.userName
                                        ] || 0) + finalDmg;
                                } else {
                                    statsRef.current.enemyDamage[u.userName] =
                                        (statsRef.current.enemyDamage[
                                            u.userName
                                        ] || 0) + finalDmg;
                                }
                            }
            ...
            function solveSocialDynamics(uA: any, uDataA: any, uB: any) {
                ...
                const comfortZone =
                    (uDataA.separationRadius || 1.1) +
                    (uDataB.separationRadius || 1.1);

                if (distSq < comfortZone * comfortZone) {
                    const dist = Math.sqrt(distSq) || 0.001;
                    const overlap = comfortZone - dist;
                    const weightA =
                        uDataA.status === "attacking" && !uDataA.isKiting
                            ? 0.0
                            : 1.0;
                    const weightB =
                        uDataB.status === "attacking" && !uDataB.isKiting
                            ? 0.0
                            : 1.0;
                    ...
                    if (totalWeight > 0) {
                        const pushStrength =
                            settingsRef.current.separationStrength *
                            (1.0 - dist / comfortZone) *
                            0.5;
                        const nx = dx / dist;
                        const nz = dz / dist;
                        ...
                        uDataA.position[0] += nx * forceA;
                        uDataA.position[2] += nz * forceA;
                        uDataB.position[0] -= nx * forceB;
                        uDataB.position[2] -= nz * forceB;

                        const vA = vehicles.current.get(uA.id);
                        const vB = vehicles.current.get(uB.id);
                        if (vA) {
                            vA.position.x = uDataA.position[0];
                            vA.position.z = uDataA.position[2];
                        }
                        if (vB) {
                            vB.position.x = uDataB.position[0];
                            vB.position.z = uDataB.position[2];
                        }
                    }
                }
            }
            ...
            // Stats sync (1000ms throttle — reduced frequency for performance)
            if (now - lastStateUpdate.current > 1000) {
                ...
                for (let i = 0; i < unitsRef.current.length; i++) {
                    const u = unitsRef.current[i];
                    if (u && !u.isDying && u.hp > 0) {
                        if (u.type === "player") _pC++;
                        else _eC++;
                    }
                }

                useStore.getState().setArmyCounts(_pC, _eC);
                // Pass refs directly — no need to clone, UI only reads
                useStore.getState().setLiveStats({
            ...
            const bottleneck = frameEngineMs > 6.0 ? "CPU/LOGIC" : currentFps < 45 ? "GPU/RENDER" : "OPTIMAL";
            ...
            if (settings.telemetry) {
                ...
                settings.telemetry.bottleneck = bottleneck;
            }
    ...
    const downloadPerfLogs = useCallback(() => {
        ...
        const lowFpsPoints = (history as any[]).filter(
            (p) => p.fps < 40,
        ).length;
        ...
        const data = {
            summary: {
                ...
                primary_suspect:
                    lowFpsPoints > cpuSpikes
                        ? "GPU/RENDERING (Too many objects)"
                        : "CPU (Logic/AI complexity)",
                advice:
                    lowFpsPoints > cpuSpikes
                        ? "Turn on Potato Mode or reduce Unit Scale"
                        : "Reduce Max Units or simplify AI behavior",
            },
            metadata: {
    ...
    return {
        ...
        updateSettingsRef: (newSettings: any) => {
