...
export default function TrainingPage() {
  ...
  if (!mounted) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center space-y-6">
        <div className="p-4 bg-indigo-500/10 rounded-3xl animate-pulse">
           <Swords className="w-12 h-12 text-indigo-500" />
        </div>
        <div className="text-zinc-500 font-black uppercase tracking-[0.4em] text-xs">Loading Training Arena...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden flex flex-col">
      <div className="absolute bottom-0 left-0 z-[9999] pointer-events-auto">
         <Leva theme={{
            colors: {
      ...
      <header className="px-8 py-6 bg-zinc-900/50 backdrop-blur-xl border-b border-white/5 flex items-center justify-between z-[100]">
        <div className="flex items-center gap-6">
           <Link 
             href="/game" 
             className="p-3 bg-zinc-800 hover:bg-zinc-700 rounded-2xl text-zinc-400 hover:text-white transition-all group"
           >
           ...
           <div className="space-y-1">
              <div className="flex items-center gap-3">
                 ...
                 <h1 className="text-2xl font-black italic tracking-tighter uppercase leading-none">Training Grounds</h1>
              </div>
        ...
        <div className="flex items-center gap-4">
           <div className="px-6 py-3 bg-zinc-800/50 rounded-2xl border border-white/5 hidden md:flex items-center gap-4">
              <div className="flex items-center gap-2">
                 <Zap className="w-4 h-4 text-amber-500" />
                 <span className="text-xs font-black text-white tabular-nums">SIMULATION READY</span>
              </div>
           </div>
           <button 
             onClick={() => window.location.reload()}
             className="p-3 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 rounded-2xl text-rose-500 transition-all flex items-center gap-2 text-xs font-black uppercase tracking-widest"
           >
      ...
      <main className="flex-1 relative flex overflow-hidden">
        ...
        <aside className="w-96 bg-zinc-950 border-l border-white/5 p-8 overflow-y-auto z-[50] space-y-10">
           <TrainingAnalytics 
           ...
           <div className="mt-10 pt-10 border-t border-white/5 space-y-6">
              <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Training Tips</h4>
              <ul className="space-y-4">
                 {[
                   ...
                   "Click 'Clear All' to reset DPS calculations."
                    ...
                    <li key={i} className="flex gap-4 p-4 bg-white/[0.02] rounded-2xl border border-white/5 text-[11px] text-zinc-400 leading-relaxed">
                       <div className="w-1.5 h-1.5 rounded-full bg-indigo-500/50 mt-1 flex-shrink-0" />
                       {tip}
                    </li>
                 ))}
              </ul>
